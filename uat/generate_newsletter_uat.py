...
# run_status.json is a PRODUCTION build artifact, and runner_common.was_truncated
# writes to it from shared code. Repoint it at the sandbox: nothing in uat/ may
# write a production file, and a UAT run leaving a stale status behind would
# make a later local verify_run.py report on the wrong run entirely.
run_status.STATUS_PATH = OUTPUT_DIR / "run_status.json"
...
def run_pass2(story_plan: str, client: anthropic.Anthropic, game_state: dict | None = None,
    ...
    for _turn in range(1, MAX_TURNS + 1):
        ...
        run_status.append("incomplete_passes", "PASS 2 (turn ceiling)")
