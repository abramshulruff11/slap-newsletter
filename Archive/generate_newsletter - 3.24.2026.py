...
Prompt assembly order:
  ...
  3. editorial_annotations.txt    — Tweet curation, story selection, tone lines (stable)
  4. rolling_feedback.txt         — Synthesized feedback from last 5-7 issues (dynamic)
"""
...
EDITOR_MODEL = "claude-sonnet-4-20250514"  # Swap to "claude-haiku-4-5-20251001" to save cost
...
def fetch_giphy_url(search_term: str, api_key: str) -> str | None:
    ...
    try:
        ...
        with urlopen(req, timeout=5) as resp:
            ...
            if results:
                # Use the downsized version for email compatibility
                images = results[0].get("images", {})
                # Prefer downsized for email/substack, fall back to original
...
def clean_giphy_search(term: str) -> list[str]:
    """
    Generate search term variants from longest to shortest.
    Giphy works best with 2-5 word queries, so we progressively simplify.
    Returns a list of queries to try in order.
    """
    # Strip quotes, parentheses, brackets
    ...
    # If it's long, also try a shorter version (first 4 words)
    words = clean.split()
    if len(words) > 4:
...
def embed_gifs_in_html(html: str, api_key: str) -> str:
    """
    Find GIF placeholder divs and replace them with actual Giphy images.
    Tries progressively shorter search terms if the first query fails.
    Falls back to keeping the placeholder if all queries fail.
    """
    ...
    for search_term in matches:
        ...
        time.sleep(0.3)  # Rate-limit Giphy calls
...
def build_system_prompt() -> list[dict]:
    ...
    # Block 3: Editorial annotations (cacheable — changes infrequently)
    ...
    if editorial_annotations:
        blocks.append({
            ...
            "text": (
                ...
                "This document teaches you HOW TO THINK about story selection, "
                "tweet curation, joke sequencing, cross-sport connections, and tone lines. "
                "Read it before writing. Apply it to every editorial decision.\n\n"
                + editorial_annotations
            ),
            "cache_control": {"type": "ephemeral"},
        })
        print("  [PROMPT] editorial_annotations.txt loaded (cached)")
    else:
        print("  [PROMPT] editorial_annotations.txt not found -- tweet curation and story selection will be generic")

    # Block 4: Rolling feedback (NOT cached — changes daily)
    if rolling_feedback:
        blocks.append({
            "type": "text",
            "text": (
...
def generate_newsletter(raw: dict) -> str:
    ...
    # Cost calculation (Sonnet: $3/MTok input, $15/MTok output)
    # API reports: input_tokens = non-cached input, cache_read = cached reads, cache_create = cache writes
    # Cached reads: $0.30/MTok (90% discount), Cache writes: $3.75/MTok (25% premium)
    base_input_cost = total_input_tokens * 3 / 1_000_000
...
def editor_pass(draft_html: str) -> str:
    """
    Second-pass editorial review. Reads the draft as a reader would
    and rewrites for flow, cohesion, and energy arc.
    """
    api_key = os.getenv("ANTHROPIC_API_KEY")
    client = anthropic.Anthropic(api_key=api_key)

    # Load editor prompt from file (editable without touching code)
    editor_prompt = load_prompt("editor_prompt.txt")
    if not editor_prompt:
        print("  [EDITOR] WARNING: prompts/editor_prompt.txt not found — skipping editor pass")
        return draft_html

    # Load voice examples and editorial annotations so the editor knows the standard
    voice_examples = load_prompt("voice_examples.txt")
    editorial_annotations = load_prompt("editorial_annotations.txt")
    ...
    # Give the editor the voice standard and editorial rules as reference
    if voice_examples:
        system_blocks.append({
            "type": "text",
            "text": (
                "## VOICE REFERENCE\n\n"
                "The following are hand-written examples of the target voice. "
                "The edited newsletter should match this energy.\n\n"
                + voice_examples
            ),
            "cache_control": {"type": "ephemeral"},
        })
    ...
    # Use same pricing as main model (adjust if using Haiku)
    if "haiku" in EDITOR_MODEL:
        input_cost = input_tokens * 0.80 / 1_000_000
        cache_read_cost = cache_read * 0.08 / 1_000_000
        cache_write_cost = cache_write * 1.0 / 1_000_000
    ...
    est_cost = input_cost + cache_read_cost + cache_write_cost + output_cost

    print(f"[EDITOR DONE]")
    print(f"[STATS] Tokens — Input: {input_tokens:,} | Output: {output_tokens:,}")
    if cache_read or cache_write:
        print(f"[CACHE] Read: {cache_read:,} | Written: {cache_write:,}")
    print(f"[COST]  Editor pass: ${est_cost:.4f}")
...
def create_substack_version(html_content: str) -> str:
    ...
    Substack natively embeds tweets when a tweet URL appears completely alone
    on its own line — no HTML tags, no surrounding text. Just the bare URL.

    This replaces our blockquote tweet format with a bare URL inside a simple
    <p> tag. When you paste this into Substack's editor, each tweet URL on
    its own line will auto-render as a full tweet card with photos and media.
    """
    # Replace blockquote tweets with just a bare URL in its own <p> tag
    blockquote_pattern = re.compile(
    ...
    def replace_with_bare_url(match):
        url = match.group(1)
        # Bare URL in its own paragraph — Substack auto-embeds this
        return f'<p>{url}</p>'
...
def save_newsletter(content: str, skip_oembed: bool = False) -> None:
    ...
    # ── Version 1: Browser preview (with oEmbed rich embeds + GIFs) ──
    ...
    if giphy_key:
        ...
        if gif_count > 0:
            ...
            print("        Get a free key at https://developers.giphy.com/dashboard/")

    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        f.write(preview_html)

    # ── Version 2: Substack-ready (raw tweet URLs + GIFs for native embedding) ──
    substack_content = create_substack_version(content)

    # Also embed GIFs in the Substack version
    if giphy_key:
...
def main() -> None:
    ...
    # Editor pass: rewrite for flow and cohesion
    if skip_editor:
        print("\n[EDITOR] Skipped (--no-editor flag)")
    else:
        content = editor_pass(content)
