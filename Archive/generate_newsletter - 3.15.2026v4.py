...
def embed_gifs_in_html(html: str, api_key: str) -> str:
    ...
    Falls back to keeping the placeholder if Giphy search fails.
    """
    ...
    for search_term in matches:
        clean_term = search_term.strip().strip('"').strip("'")
        print(f"  [GIPHY] Searching: {clean_term}")
        gif_url = fetch_giphy_url(clean_term, api_key)
        ...
        if gif_url:
            ...
            img_html = (
                ...
                f'<img src="{gif_url}" alt="{clean_term}" '
            ...
            print(f"           → No results, keeping placeholder")
